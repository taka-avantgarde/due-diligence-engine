<div align="center">

# Due Diligence Engine

**IDEのAIが、デューデリジェンスアナリストになる。**

`dde prompt` をターミナルで実行 — Claude Code、Cursor、Copilotがコードベースを読み、投資判断レベルの評価レポートを生成。APIキー不要。追加コスト¥0。

より深い分析には、Claude・Gemini・ChatGPTの**3社並列クロス検証**も対応。

[![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)
[![Python](https://img.shields.io/badge/Python-3.11+-green.svg)](https://python.org)
[![AI](https://img.shields.io/badge/AI-Claude_%7C_Gemini_%7C_ChatGPT-orange.svg)](https://github.com/taka-avantgarde/due-diligence-engine)
[![IDE](https://img.shields.io/badge/IDE_AI-Claude_Code_%7C_Cursor_%7C_Copilot-blueviolet.svg)](https://github.com/taka-avantgarde/due-diligence-engine)

[English](README.md) | [日本語](README.ja.md)

</div>

---

## 2つの使い方

| | 方法 | 内容 | 費用 |
|---|------|------|------|
| 🖥️ | **AIターミナルから使う**（Claude Code / Cursor / Copilot） | `dde prompt --pdf` を実行 → AIがコードを読み、評価し、コンサルティンググレードPDFを自動生成 | 無料（APIキー不要） |
| 🌐 | **Webダッシュボードから使う** | GitHub URLを貼るだけ → マルチAI（Claude + Gemini + ChatGPT）クロス検証 | 無料（BYOK） |

> **どちらを使うべき？** AI搭載IDEをお使いなら `dde prompt --pdf` が最速 — セットアップ不要、追加コスト¥0、PDFがローカルに生成されます。Webダッシュボードは複数AIによるクロス検証や、共有可能なレポートが必要な場合に最適です。

---

## マルチAIクロス検証

```
         ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
         │   Claude     │  │   Gemini    │  │  ChatGPT    │
         │  (Anthropic) │  │  (Google)   │  │  (OpenAI)   │
         └──────┬───────┘  └──────┬──────┘  └──────┬──────┘
                │                 │                 │
                └────────┬────────┴────────┬────────┘
                   ┌─────▼─────────────────▼─────┐
                   │   クロス検証エンジン          │
                   └──────────────┬───────────────┘
                         ┌────────▼────────┐
                         │  統合スコア       │
                         │  6次元評価        │
                         └─────────────────┘
```

---

## まず試してみる

**最速の方法** — AIターミナル（Claude Code、Cursor等）で:
```bash
pip install --no-cache-dir git+https://github.com/taka-avantgarde/Due-diligence-engine.git
dde prompt --pdf --lang ja
```

これだけです。言語選択（1: English, 2: 日本語）が最初に表示されます。カレントディレクトリを自動で分析。AIがコードを読み、評価し、PDFを生成します。全自動。

```bash
# GitHubリポを指定する場合
dde prompt owner/repo --pdf --lang ja
```

**Webダッシュボード**: https://due-diligence-engine.web.app/dashboard/

> `dde prompt` は無料（APIキー不要）。`--pdf` でコンサルティンググレードPDFを自動生成。マルチAIクロス検証には `dde analyze` + 自分のAPIキー — **BYOK（Bring Your Own Key）**: Claude・Gemini・ChatGPT対応。

---

## 料金プラン

| プラン | 費用 | AIプロバイダー | 機能 |
|--------|------|---------------|------|
| **無料（ローカルのみ）** | 無料 | なし | コード構造、Git履歴、依存関係スキャン |
| **BYOK** | 無料（API費用は自己負担） | Claude / Gemini / ChatGPT（1〜3社） | フルAI分析 + クロス検証 |
| **Pro分析（日本のみ）** | **¥20,000 / 社** | Claude + Gemini（当社管理） | AI自動レポート + オンライン会議サポート |

> **BYOK:** APIキー1つから始められます。プロバイダーを追加するとクロス検証が自動有効化。

> **Pro分析:** 1社¥20,000で自動レポート生成 + オンライン会議でリアルタイムサポート。お気軽にお問い合わせください。
>
> **[Atlas Associates](https://www.atlasassociates.io/)** — support@atlasassociates.io

---

## 機能一覧

| 機能 | 説明 |
|------|------|
| **マルチAIクロス検証** | Claude + Gemini + ChatGPTが独立評価→クロス検証 |
| **`dde prompt --pdf` — コンサルティングPDF** | AIがコードを読み→SWOT・スコアリング・将来性・投資判断→PDFを自動生成。APIキー不要 |
| **`dde prompt` — IDE AI連携** | Claude Code / Cursor / Copilot向け構造化プロンプト生成。APIキー不要 |
| **BYOK（Bring Your Own Key）** | 自分のAPIキーで分析。Claude・Gemini・ChatGPT対応。1社でも3社でも |
| **ほんやくコンニャク（用語集）** | 全技術用語に比喩付き注釈。IT業界外の投資家・事業会社でもレポートが読める |
| **マッチ率表示** | 主張 vs コード実態 — ステータスバーでチームの誠実さを可視化 |
| **GitHub Privateリポ対応** | PATベースの一時アクセス |
| **サイト検証（10項目）** | 機能主張一致度、技術スタック整合性、セキュリティ主張検証、パフォーマンス主張妥当性、規模主張一貫性、チーム規模推定、ローンチ日検証、料金モデル実現性、コンプライアンス表示監査、AIウォッシュ指数 |
| **競合分析チャート（7種類）** | Forrester Wave、BCG成長・シェア、McKinsey技術モート、セキュリティ＆プライバシー成熟度、データガバナンス＆透明性、GSリスク・リターン、イノベーションバブル — 6〜16社×6市場横断、軸選定理由の説明付き |
| **AIウォッシュ検出** | 「独自AI」と偽るAPIラッパーを検出 |
| **Git履歴フォレンジック** | コミット履歴の不審パターン（DD直前の急造など）を分析 |
| **10段階技術レベル評価** | 各軸Lv.1〜10で明確な基準付き |
| **ステージ別評価基準** | シード / シリーズA / シリーズB / グロース — ステージに合わせた基準 |
| **PDF出力** | 投資委員会向けプロフェッショナルPDF |
| **切断 & 破棄** | ワンクリックでデータ消去 + 破棄証明書 |
| **バイリンガル** | English / 日本語 — CLI（`--lang`）& ダッシュボード |

---

## スコアリング

### 最終スコアの算出式

```
最終スコア = ヒューリスティック分析 (30%) + AI平均スコア (70%)
```

### スコアバロメーター

```
  0          40          60          75          90        100
  |----------|-----------|-----------|-----------|----------|
  F          D           C           B           A
  投資不可   高リスク    懸念あり    条件付き    有力候補
                                     投資可能
```

| スコア | 評価 | 推奨アクション |
|--------|------|----------------|
| 90–100 | 🏆 **A** | 有力な投資候補 |
| 75–89  | ✅ **B** | 条件付きで投資可能 |
| 60–74  | ⚡ **C** | 重要な懸念あり — 精査が必要 |
| 40–59  | ⚠️ **D** | 高リスク |
| 0–39   | 🚫 **F** | 投資不可 |

---

### 6軸評価

| 評価軸 | 重み | 検出内容 |
|--------|------|----------|
| 技術独自性 | 25% | APIラッパー vs 本物のIP |
| 技術先進性 | 20% | 技術スタックの先進度 |
| 実装深度 | 20% | PoC vs 本番品質 |
| アーキテクチャ品質 | 15% | 設計品質 |
| 主張整合性 | 10% | ピッチ vs 現実 |
| セキュリティ態勢 | 10% | セキュリティ成熟度 |

---

### 10段階技術レベル評価

各軸は **Lv.1〜10** の明確な基準でレーティング：

```
Lv.1 ──── Lv.2 ──── Lv.3 ──── Lv.4 ──── Lv.5 ──── Lv.6 ──── Lv.7 ──── Lv.8 ──── Lv.9 ──── Lv.10
 ▓          ▓▓         ▓▓▓        ▓▓▓▓       ▓▓▓▓▓      ▓▓▓▓▓▓     ▓▓▓▓▓▓▓    ▓▓▓▓▓▓▓▓   ▓▓▓▓▓▓▓▓▓  ▓▓▓▓▓▓▓▓▓▓
最低水準                                      業界標準                                                最高水準
```

| レベル | 技術独自性 | 実装深度 | アーキテクチャ品質 |
|--------|-----------|---------|-----------------|
| Lv.1  | コピー・独自コードなし | UIモックのみ | スパゲッティコード |
| Lv.3  | 複数API組み合わせ | 動くプロトタイプ | 基本的なレイヤー分離 |
| Lv.5  | 既存技術の大幅拡張＋独自アルゴリズム一部 | Beta・基本テストあり | 関心の分離が明確 |
| Lv.7  | コア技術が独自実装 | 本番運用・監視完備 | マイクロサービス |
| Lv.10 | 世界最先端・既存技術を置き換える可能性 | ミッションクリティカル | 分散システム |

---

### スコアリング例 ―「NeuralPay」（フィンテックAIスタートアップ）

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  DUE DILIGENCE ENGINE  ·  NeuralPay / neuralpay/core-api
  分析AI: Claude + Gemini + ChatGPT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  総合スコア                             評価
  ┌────────────────────────────┐         ┌───────┐
  │  ████████████████░░░░  78  │         │   B   │
  └────────────────────────────┘         └───────┘
  ▲ 条件付きで投資可能

  スコアバロメーター:
  0          40          60         [75]        90        100
  |----------|-----------|-----------|----●------|----------|
  投資不可   高リスク    懸念あり    条件付き    有力候補
                                     ^ここ

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  6軸評価
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  技術独自性         (25%)   Lv.7 / 10
  ████████████████████░░░░░░░░░░  72   ← 独自MLモデル、APIラッパーではない

  技術先進性         (20%)   Lv.8 / 10
  ████████████████████████░░░░░░  84   ← Rust + TypeScript、先進スタック

  実装深度           (20%)   Lv.6 / 10
  ██████████████████░░░░░░░░░░░░  61   ← Beta段階、テストはあるが薄い

  アーキテクチャ品質 (15%)   Lv.7 / 10
  ████████████████████░░░░░░░░░░  73   ← クリーンなマイクロサービス、一部ギャップ

  主張整合性         (10%)   Lv.5 / 10
  ██████████████░░░░░░░░░░░░░░░░  54   ⚠ 「SOC2取得」を主張、コードに証拠なし

  セキュリティ態勢   (10%)   Lv.6 / 10
  ██████████████████░░░░░░░░░░░░  62   ← TLS完備、ペネトレーションテスト未実施

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  AIプロバイダー別スコア
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Claude   (Anthropic)  ████████████████████░░░░░░  80
  Gemini   (Google)     ███████████████████░░░░░░░░  77
  ChatGPT  (OpenAI)     ████████████████████░░░░░░░  79
  ─────────────────────────────────────────────────────
  AIコンセンサス                                  78.7  ✓ 3社の評価が一致

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  レッドフラグ（2件）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  ⚠️  [MEDIUM] コンプライアンス主張の矛盾
      ピッチ資料: 「SOC2 Type II 認定取得済み」
      コードベース: 監査ログなし、コンプライアンスツール未導入

  ⚠️  [LOW]    Git履歴: コミットの40%がDD直前2週間に集中
               DD前の急造クリーンアップの可能性

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  推奨アクション
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  ✅ B — 条件付きで投資可能
  技術スタックとコアMLモデルは正真正銘の独自実装。
  SOC2主張の不一致を解消し、テストカバレッジを改善した上で
  タームシートに進むことを推奨。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

---

## クイックスタート

```bash
git clone https://github.com/taka-avantgarde/due-diligence-engine.git
cd due-diligence-engine
pip install -e .
```

### 設定

```bash
# AIプロバイダー（1社、2社、または3社全て設定可能）
export ANTHROPIC_API_KEY="sk-ant-..."     # Claude
export GOOGLE_AI_API_KEY="AIza..."        # Gemini
export OPENAI_API_KEY="sk-..."            # ChatGPT
```

### CLI

```bash
# コンサルティンググレードPDF生成（AIターミナル内 — Claude Code、Cursor等）
dde prompt --pdf --lang ja                          # カレントディレクトリ
dde prompt owner/repo --pdf --lang ja --stage seed  # GitHubリポ

# 非対話モード（AIターミナルで対話入力が使えない場合）
dde prompt --pdf --lang ja --url https://example.com --url https://docs.example.com

# 評価プロンプトのみ生成（PDF不要の場合）
dde prompt
dde prompt owner/repo --lang ja --stage seed

# AI API使用のフル分析（BYOK）
dde analyze https://github.com/some-startup/their-product

# ローカルのみ（無料、AIなし）
dde analyze some-startup/repo --skip-ai

# AI評価JSONからPDF生成（--pdfモードが内部で使用）
dde report --consulting result.json --pdf --lang ja
```

---

## AIターミナルから使う（Claude Code / Cursor 等）

### 1コマンドでPDF生成

```bash
pip install --no-cache-dir git+https://github.com/taka-avantgarde/Due-diligence-engine.git
dde prompt --pdf --lang ja
```

カレントディレクトリを自動で分析。リポ指定も可: `dde prompt owner/repo --pdf`

AIが自律的に:
1. コードベースを読み取り
2. 世界トップクラスのテクノロジーコンサルタントとして評価
3. コンサルティンググレードPDFを `~/Downloads/` に生成

APIキー不要。言語選択（1/2）が最初に表示され、あとはIDEのツール同意プロンプトのみ。

**出力先:** `~/Downloads/dde_consulting_<プロジェクト名>_<日付>.pdf`
- 日本語: `dde_consulting_NeuralPay_2026年04月02日.pdf`
- 英語: `dde_consulting_NeuralPay_2026-04-02.pdf`

### PDFに含まれる内容

| ページ | セクション | 内容 |
|--------|-----------|------|
| 1 | **表紙** | プロジェクト名、総合スコア、グレードバッジ |
| 2 | **スコアダッシュボード** | 総合スコア（大型表示）+ 6次元の横棒グラフ（各軸の評価基準説明付き）+ スコアバロメーター（F→A） |
| 3 | **ビジネスサマリー** | 非エンジニアにも読める要約 + 投資評価バッジ + 使用AIモデル |
| 4 | **SWOT分析** | 強み・弱み・機会・脅威 — エビデンスベース、ビジネスアナロジー付き |
| 5 | **スコア内訳** | 6次元の詳細テーブル。根拠・ビジネス説明・可能になること付き |
| 6 | **テクレベル評価** | 視覚的ゲージバー（Lv.1-10）+ 平易な解説 |
| 7 | **将来性評価** | プロダクトビジョン + 1/3/5年予測（信頼度・マイルストーン付き） |
| 8 | **戦略アドバイス** | 即座のアクション、中期優先事項、長期ビジョン |
| 9 | **投資判断** | 推奨度 + リスク/アップサイド + 類似企業 |
| 10 | **レッドフラグ** | 深刻度別の問題点 + ビジネスインパクト |
| 11 | **サイト検証** | プロダクト/サービスURLに対する10項目の信頼性監査（URL入力時のみ） |
| 12-14 | **競合分析（7チャート）** | Forrester Wave、BCGマトリックス、McKinsey技術モート、セキュリティ＆プライバシー成熟度、データガバナンス＆透明性、GSリスク・リターン、イノベーションバブル — 6〜16社×6市場、軸説明キャプション付き |
| 13 | **用語集** | 全技術用語に非技術者向けの注釈 |

### プロンプトのみモード（PDF不要の場合）

`--pdf` を省略するとテキスト出力のみ:

```bash
dde prompt owner/repo --lang ja

# ファイル保存 / クリップボードにコピー
dde prompt . -o prompt.md
dde prompt . --copy
```

### `--pdf` モードの仕組み

```
┌──────────────────────────────────────────────────────────┐
│  あなたのIDE（VS Code / JetBrains 等）                   │
│                                                          │
│  ┌──────────────────────────────────┐                    │
│  │  AIターミナル                     │                    │
│  │  (Claude Code / Cursor 等)       │                    │
│  │                                  │                    │
│  │  $ dde prompt --pdf              │  ┌──────────────┐  │
│  │      ↓                           │  │ DDE Engine   │  │
│  │  1. データ収集（ローカル）────────┼─▶│（AI API不使用 │  │
│  │     + JSONスキーマ + 実行指示    │  │  ローカルのみ）│  │
│  │      ↓                           │  └──────────────┘  │
│  │  2. AIがコードを読み、評価        │                    │
│  │      ↓                           │                    │
│  │  3. JSON保存 + 実行:             │                    │
│  │     dde report --consulting ─────┼─▶ PDF生成          │
│  │      ↓                           │                    │
│  │  4. PDFファイルパスを表示         │                    │
│  └──────────────────────────────────┘                    │
│                                                          │
│  追加コスト: ¥0 — 既存のAIサブスクで分析                  │
└──────────────────────────────────────────────────────────┘
```

> **追加コスト¥0** — `dde prompt` は完全ローカル実行（AI APIを呼びません）。`--pdf` モードでは、AIが構造化JSONを生成し、`dde report --consulting` でプロフェッショナルPDFを出力します。直接API呼び出しが必要な場合は `dde analyze`（BYOK対応）も利用可能。

---

## Privateリポジトリアクセス（PAT）

1. GitHub → Settings → Developer settings → Personal access tokens → Tokens (classic)
2. Generate new token → **`repo`** スコープにチェック → 有効期限7日
3. トークン（`ghp_...`）をダッシュボードに貼り付け

**セキュリティ:** PATはメモリ上で `git clone` に1回使用後、即座に破棄。保存されません。

---


## データセキュリティ

| 保証事項 | 実装方法 |
|---------|---------|
| クラウド保存なし | 暗号化tmpfsで処理 |
| レポートにコードなし | PDFには所見のみ |
| 暗号消去 | 3パスランダム上書き + 破棄証明書 |
| API 0-day保持 | Anthropic / Google / OpenAI — データ保持なし |

---

## デプロイ

| 方式 | 費用 | 説明 |
|------|------|------|
| `dde prompt --pdf` | 無料 | IDE AIが評価 + コンサルティングPDF生成 — APIキー不要 |
| `dde prompt` | 無料 | IDE AIが評価（テキスト出力）— APIキー不要 |
| ローカルCLI | 無料 | `dde analyze owner/repo --skip-ai` |
| BYOK CLI | 無料 + API費用 | 自分のAPIキーでフルAI分析 |
| BYOKダッシュボード | 無料 + API費用 | Web UI + GitHub PAT対応 |

---

## ロードマップ

- [x] マルチAI分析エンジン（Claude / Gemini / ChatGPT BYOK）
- [x] BYOKダッシュボード
- [x] プロバイダー別スコア比較
- [x] バイリンガルUI（English / 日本語）
- [x] PDFレポート出力
- [x] 切断 & 破棄 + 証明書
- [x] `dde prompt` — IDE AI連携（Claude Code / Cursor / Copilot）
- [x] `dde prompt --pdf` — コンサルティンググレードPDF（SWOT・将来性・投資判断）
- [x] スコアダッシュボード（横棒グラフ + スコアバロメーター）
- [x] PDF出力先 `~/Downloads/` + 日付付きファイル名（日英対応）
- [x] EN/JA PDF自動生成テスト（13テスト）
- [x] プロフェッショナルなグレー/ダークブルーカラー + Atlas Associatesクレジット
- [x] 数字選択式の言語選択（1: English, 2: 日本語）— 分析開始前に必ず表示
- [x] 各評価軸バーに評価基準の説明を追加
- [x] 日本語テーブル・ゲージのCIDフォントレンダリング修正
- [x] ほんやくコンニャク（非技術者向け用語集）
- [x] マッチ率表示（主張 vs コード）
- [x] ステージ別評価基準（seed / series_a / series_b / growth）
- [x] 投資家向け質問自動生成
- [x] サイト検証 — プロダクト/サービスURLに対する10項目の信頼性監査（最大3URL）
- [x] 競合分析チャート — コンサルファーム級7種類×6グローバル市場（6〜16社）
- [x] セキュリティ＆プライバシー成熟度 + データガバナンス＆透明性チャート追加
- [x] 各競合チャートページに軸選定理由の説明キャプション
- [x] JSONスキーマに `project_name` フィールド追加（PDF表紙・ファイル名に自動使用）
- [x] 非対話CLIモード（`--url` フラグ + `--lang` フラグでAIターミナル対応）
- [x] PDF表紙に使用AIモデル表示
- [x] ダークテーマ表紙 + Atlas Associatesブランディング
- [x] PDFレイアウト修正 — 表紙・スコアダッシュボードのテキスト重なり解消
- [x] 17自動テスト（EN/JA PDF + サイト検証 + 競合分析）
- [ ] 技術的負債 + 保守性の評価軸追加
- [ ] バッチ分析（ポートフォリオ全体DD）
- [ ] 時系列トラッキング（再分析による推移確認）

---

## 免責事項

本ツールは**投資判断を支援するための技術分析**を提供します。投資助言ではありません。投資判断には必ず専門家にご相談ください。

---

## ライセンス

[Apache License 2.0](LICENSE) — Copyright 2026 Takayuki Miyano / Atlas Associates

---

<div align="center">

**Powered by Due Diligence Engine — Takayuki Miyano / Atlas Associates**

Built with Claude (Anthropic) + Gemini (Google) + ChatGPT (OpenAI)

</div>
