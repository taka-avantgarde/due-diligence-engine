...
> **注意:** ホスティング版のデモは、AIなしの**簡易チェック**（ローカルコード分析のみ）です。Claudeによる正確で詳細なAI分析（Haiku → Sonnet → Opusパイプライン）を利用するには、ご自身のClaude APIキーを設定してください。[設定方法](#設定)を参照（BYOK: Bring Your Own Key）。
