...
**Q: スコア重みをカスタマイズできますか?**
A: 5 次元の重みは各 20% 均等（バランス・簡潔・解釈容易）。Atlas 4 軸の重み（25/20/5/50）は Atlas 哲学を反映したもので固定。Security Strength 内のサブ重みは業界コンテキストにより動的調整。
...
- ✅ 訪問者カウンターバッジ（komarev + hits.seeyoufarm）
- ✅ PDF レイアウト: KeepTogether、6 次元 Lv バー、SWOT 2×2 グリッド

**以前のリリース (v0.2.0)**
- ✅ Atlas 4 軸最適化評価（「セキュリティ強度」50%）
- ✅ 実装能力マトリックス（第8競合チャート）
